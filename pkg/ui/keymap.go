...
// topLevelBindings is the full top-level keymap. "Top-level" means keys
// that apply when no overlay is open and the user isn't in search-input
// mode — overlays define their own local keymaps in their handlers.
//
...
//
// Handlers take Model as a parameter rather than closing over a receiver,
// so the slice can live at package scope — dispatched on every keystroke
// without allocating.
var topLevelBindings = []Binding{
	{
...
// bindings returns the top-level keymap. Method form preserved for ergonomics
// at the callsites (overlay_help and dispatch).
func (m Model) bindings() []Binding { return topLevelBindings }
...
func (m Model) dispatchBinding(key string) (Model, tea.Cmd, bool) {
	for _, b := range topLevelBindings {
